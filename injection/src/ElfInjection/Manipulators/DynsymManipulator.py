...
class ElfDynsymManipulator(ABC):
    ...
    def __init__(
        ...
        self.__symbol = ElfSymbolEntry(
            name, value, size, binding, visibility, symType, shndx
        )

    @abstractmethod
    def _manipulateDynsym(self, inj) -> Symbol:
        """Manipulates .dynsym
...
class ElfDynsymOverwriter(ElfDynsymManipulator):
    ...
    def __init__(
        ...
        """
        super().__init__(
            newName, newValue, newSize, newBinding, newVisibility, newSymType, newShndx
        )
    ...
    def _manipulateDynsym(self, inj) -> Symbol:
        ...
        if not sym:
            raise RuntimeError(
                "Symbol with name {} not found.".format(self.__targetName)
            )
...
class ElfDynsymInserter(ElfDynsymManipulator):
    ...
    def _manipulateDynsym(self, inj) -> Symbol:
        ...
        """
        elfbin = inj.getElfBinary()
        return elfbin.getBinary().add_dynamic_symbol(self._getSymbol().asSymbol())
