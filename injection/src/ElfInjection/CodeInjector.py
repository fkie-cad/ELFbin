...
from .Manipulators.DynamicManipulator import ElfDynamicEntry
...
class ElfCodeInjector:
	...
	def injectCodeCave(
		    self,
		    phtManip : ElfPhtManipulator,
		    codeCave : ElfCodeCave,
		    data : bytes) -> Tuple[ElfShellcode, Segment]:
		...
		def findLoadable(b : Binary, cave : ElfCodeCave):
			...
			loadables = [
				(
					seg,
					cave.offset - seg.file_offset
				)
				for seg in b.segments
				if (seg.type == SEGMENT_TYPES.LOAD and
					cave.offset >= seg.file_offset)
			]
			if loadables:
		...
		end = binary.segment_from_offset(
			codeCave.offset + codeCave.size - 1
		)
		...
		if (not start and not end):
			...
			# over two segments.
			elif (start.type == SEGMENT_TYPES.LOAD and
					end.type == SEGMENT_TYPES.LOAD):
	...
	def findCodeCaves(
		    self,
		    caveSeeker : ElfCodeCaveSeeker) -> List[ElfCodeCave]:
	...
	def injectSegment(
			self,
			phtManip : ElfPhtManipulator,
			memManip : ElfMemoryManipulator,
			useLief=False) -> Segment:
		...
		if (useLief):
			...
			if (isinstance(phtManip, ElfPhtAppender) and
				isinstance(memManip, ElfMemoryInserter)):
			...
			elif (isinstance(phtManip, ElfPhtOverwriter) and
				isinstance(memManip, ElfMemoryOverwriter)):
				# Construct segment to replace with
				seg = phtManip._getEntry().asSegment()
				seg.content = list(memManip._getData())

				# Find segment to replace
				target = self.__bin.getBinary().segments[
					phtManip._getIndex()
				]
			...
			memManip._manipulateMemory(
				self,
				updatedOffset=segment.file_offset
			)
			return self.__bin.getBinary().segments[index]
		
	def overwriteEntryPoint(
			self,
			newEntryPoint : int) -> int:
	...
	def injectDynsym(
			self,
			dynsymManip : ElfDynsymManipulator) -> Symbol:
	...
	def injectDynamic(
			self,
			stringManip : ElfStringManipulator,
			dynamicManip : ElfDynamicManipulator
		) -> DynamicEntry:
		...
		"""
		if (isinstance(stringManip, ElfStringInserter) and
			isinstance(dynamicManip, ElfDynamicInserter)):
			return self.__bin.getBinary().add_library(
				stringManip._getString()
			)
		elif (isinstance(stringManip, ElfStringInserter) and
			isinstance(dynamicManip, ElfDynamicOverwriter)):
			self.__bin.getBinary().remove(
				self.__bin.getBinary().dynamic_entries[
				dynamicManip._getIndex()])
			return self.__bin.getBinary().add_library(
				stringManip._getString()
			)
		...
		result = None
		if (stringManip):
			result = stringManip._manipulateString(
				self
			)

		return dynamicManip._manipulateDynamic(
			self,
