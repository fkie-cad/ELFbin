...
class TestDynamicManipulator:
    ...
    def test_overwrite_dynamic(self, inj_arm_android_bin, string_data, index):
        binary = inj_arm_android_bin.getElfBinary()
        overwriter = ElfDynamicOverwriter(lief.ELF.DYNAMIC_TAGS.NEEDED, 4, index)

        old = binary.getBinary()
        dyn = overwriter._manipulateDynamic(
            inj_arm_android_bin, updatedString=string_data
        )
        new = binary.getBinary()

        assert len(old.dynamic_entries) == len(new.dynamic_entries)
    ...
    def test_insert_dynamic(self, inj_arm_android_bin, entry, string_data):
        binary = inj_arm_android_bin.getElfBinary()
        ...
        assert old_num == len(new.dynamic_entries) - 1
