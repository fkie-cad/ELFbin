...
class TestDynsymManipulator:
    ...
    def test_overwrite_dynsym(self, inj_arm_android_bin, symbols, symIndex):
        ...
        # Perform injection
        sym = overwriter._manipulateDynsym(inj_arm_android_bin)
        ...
        assert binary.getBinary().get_symbol(symbol.name).value != 0

    @pytest.mark.symbol
    @pytest.mark.parametrize("symIndex", range(1))
    def test_insert_dynsym(self, inj_arm_android_bin, symbols, symIndex):
        symbol = symbols[symIndex]
        binary = inj_arm_android_bin.getElfBinary()
        ...
        # Perform injection
        sym = inserter._manipulateDynsym(inj_arm_android_bin)
