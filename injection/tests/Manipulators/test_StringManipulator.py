...
class TestStringManipulator:
    ...
    def test_string_finder(self, inj_arm_android_bin, defaultLength):
        ...
        finder = ElfStringFinder(defaultLength=defaultLength)

        string_info = finder._manipulateString(inj_arm_android_bin)

        # defaultLength can never by less than 1!
        if defaultLength == 0:
            defaultLength = 1
        assert string_info.index == libcEnd - defaultLength
        assert string_info.string == libc[-defaultLength:] + ".so"
