...
class TestElfCodeInjector:
	...
	def test_inject_code_cave(
		...
		seg = lief_arm_android_bin_second.segment_from_offset(
			sc.offset
		)
		seg2 = lief_arm_android_bin_second.segment_from_offset(
			sc.offset + sc.size - 1
		)
		...
		if (seg.file_offset < sc.offset):
			...
			assert(
				seg.content[
					sc.offset - seg.file_offset
				] == ord(b'\x42')
			)
			assert(
				seg.content[
					sc.offset - seg.file_offset + sc.size - 1
				] == ord(b'\x42')
			)

			# Check sizes
			assert(
				old.physical_size + sc.size - overlap
					== seg.physical_size
			)
			assert(
				old.virtual_size + sc.size - overlap
					== seg.virtual_size
			)
		...
		# can return segment that is not meant.
		elif (seg.file_offset == sc.offset and
			  seg.physical_size > sc.size):
			...
			if not old:
				...
				# -> must be 'old != None'
				old = lief_arm_android_bin.segment_from_offset(
					sc.offset + sc.size
				)
			...
			if (overlap < 0):
				overlap = 0
			assert(
				old.file_offset - sc.size + overlap
					== seg.file_offset
			)
			assert(
				old.physical_size + sc.size - overlap
	...
	def test_inject_segment(
		...
		if useLief:
			if ((isinstance(phtManip, ElfPhtAppender) and
				isinstance(memoryManip, ElfMemoryOverwriter)) or
				(isinstance(phtManip, ElfPhtOverwriter) and
				isinstance(memoryManip, ElfMemoryInserter))):
		...
		seg = inj_arm_android_bin.injectSegment(
			phtManip,
			memoryManip,
			useLief=useLief
		)
		...
		assert(
			seg.virtual_address 
				== newSeg.virtual_address
		)
		assert(seg.physical_size == newSeg.physical_size)
		assert(
			seg.virtual_size == newSeg.virtual_size
		)
		assert(
			seg.physical_address
				== newSeg.physical_address
		)
		...
		assert(seg.content[0] == ord(b'\x42'))
		assert(
			seg.content[seg.physical_size - 1]
				== ord(b'\x42')
		)
		...
		if isinstance(phtManip, ElfPhtOverwriter):
			...
			if isinstance(memoryManip, ElfMemoryOverwriter):
				...
				assert(
					oldBinary.eof_offset + seg.physical_size
						== binary.eof_offset
				)
			else:
				assert(False)
	...
	def test_inject_dynamic(
		...
		if ((isinstance(stringManip, ElfStringInserter) and
			isinstance(dynamicManip, ElfDynamicInserter)) or
			(isinstance(stringManip, ElfStringInserter) and
			isinstance(dynamicManip, ElfDynamicOverwriter))):
			pytest.skip('Assume LIEF is correct.')

		# Perform injection
		dyn = inj_arm_android_bin.injectDynamic(
			stringManip,
			dynamicManip
		)
