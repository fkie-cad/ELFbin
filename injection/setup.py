...
def main():
    ...
    sources = []
    for f in glob.glob(rawelf_module_base + "**/*.c", recursive=True):
        sources.append(f)

    include_dirs = []
    for i in glob.glob(rawelf_module_base + "**/include/", recursive=True):
        include_dirs.append(i)

    headers = []
    for i in glob.glob(rawelf_module_base + "**/*.h", recursive=True):
        headers.append(i)
